...
void processor_t::disasm_new(insn_t insn)
{
  uint64_t bits = insn.bits();
  if (last_pc != state.pc || last_bits != bits) {
    std::stringstream s;  // first put everything in a string, later send it to output
    ...
    s << "core " << std::dec << std::setfill(' ') << std::setw(3) << id
      ...
      << " (0x" << std::setw(8) << bits << ") ["
      << "0x" << state.mstatus->read() << ","
      << std::setw(1) << state.frm->read() << ","
      << std::setw(2) << state.fflags->read() << ","
      << std::setw(2) << state.mcause->read() << ","
      << std::setw(2) << state.scause->read() << ","
      << std::setw(8) << state.medeleg->read() << ","
      << std::setw(8) << state.mcounteren->read() << ","
      << std::setw(8) << state.scounteren->read() << ","
      << std::setw(8) << state.dcsr->read() << "] "
      << disassembler->disassemble(insn) << std::endl;
    ...
    last_csr_values["frm"] = state.frm->read();
    last_csr_values["fflags"] = state.fflags->read();
    last_csr_values["mcause"] = state.mcause->read();
    last_csr_values["scause"] = state.scause->read();
    last_csr_values["medeleg"] = state.medeleg->read();
    last_csr_values["mcounteren"] = state.mcounteren->read();
    last_csr_values["scounteren"] = state.scounteren->read();
    last_csr_values["dcsr"] = state.dcsr->read();
  } else {
    executions++;
  }
}
