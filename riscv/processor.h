...
public:
  ...
  // 新增函数
  void set_coverage(bool value) { coverage = value; }
  bool get_coverage() const { return coverage; }
...
public:
  ...
  std::string last_disasm_output;  // 保存上一条指令的disasm输出
  std::map<std::string, reg_t> last_csr_values;  // 保存上一条指令的CSR值
};
